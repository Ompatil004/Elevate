...
class ModelEvaluator:
    ...
    def evaluate_workout_difficulty_model(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        metrics = self.evaluate_classification_model(y_true, y_pred)
    ...
    def evaluate_workout_ranking_model(self, y_true: np.ndarray, y_pred: np.ndarray,
                                    user_profiles: List[Dict[str, Any]], 
                                    model_outputs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def evaluate_nutrition_adherence_model(self, y_true: np.ndarray, y_pred: np.ndarray,
                                         user_profiles: List[Dict[str, Any]], 
                                         model_outputs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        # Get predictions (for this example, we'll use y_pred as-is)
        # Calculate classification metrics
        metrics = self.evaluate_classification_model(y_true, y_pred)
...
def create_sample_evaluation_data():
    ...
    # Sample test data
    y_difficulty_true = np.array(['same', 'increase', 'decrease'])
    y_difficulty_pred = np.array(['same', 'same', 'decrease'])  # Example predictions
    
    y_ranking_true = np.array([0.7, 0.9, 0.5])
    y_ranking_pred = np.array([0.65, 0.88, 0.48])  # Example predictions
    
    y_adherence_true = np.array([1, 0, 1])
    y_adherence_pred = np.array([1, 0, 1])  # Example predictions
    
    return {
        'user_profiles': user_profiles,
        'model_outputs': model_outputs,
        'y_difficulty_true': y_difficulty_true,
        'y_difficulty_pred': y_difficulty_pred,
        'y_ranking_true': y_ranking_true,
        'y_ranking_pred': y_ranking_pred,
        'y_adherence_true': y_adherence_true,
        'y_adherence_pred': y_adherence_pred
    }
...
def main():
    ...
    difficulty_results = evaluator.evaluate_workout_difficulty_model(
        y_true=sample_data['y_difficulty_true'],
        y_pred=sample_data['y_difficulty_pred'],
    ...
    ranking_results = evaluator.evaluate_workout_ranking_model(
        y_true=sample_data['y_ranking_true'],
        y_pred=sample_data['y_ranking_pred'],
        user_profiles=sample_data['user_profiles'],
        model_outputs=sample_data['model_outputs']
    )
    ...
    adherence_results = evaluator.evaluate_nutrition_adherence_model(
        y_true=sample_data['y_adherence_true'],
        y_pred=sample_data['y_adherence_pred'],
        user_profiles=sample_data['user_profiles'],
        model_outputs=sample_data['model_outputs']
    )
