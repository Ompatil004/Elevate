...
class OfflineRetrainer:
    ...
    def _prepare_difficulty_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        difficulty_outcomes = [entry for entry in outcome_data if entry['model_name'] == 'workout_difficulty']
        
        # Combine feedback and outcomes to create training data
        X, y = [], []
    ...
    def retrain_model(self, model_name: str, force_retrain: bool = False) -> bool:
        ...
        if not force_retrain and not self._has_sufficient_data_for_retraining(model_name):
            self.logger.info(f"Not enough new data for retraining {model_name}")
            return False
        ...
        try:
            ...
            current_model = self.version_manager.load_model(model_name, current_version)
            current_metrics = self._validate_model(current_model, model_name, X_val, y_val)
            ...
            if should_deploy:
                ...
                self.logger.info(f"Deployed new model {model_name} version {new_version}")
                return True
            else:
                self.logger.info(f"New model for {model_name} did not outperform current model, not deploying")
                return False
    ...
    def _has_sufficient_data_for_retraining(self, model_name: str) -> bool:
        """Check if we have sufficient new data for retraining"""
        # For this example, we'll say we need at least 50 new data points
        # In practice, this would depend on your specific requirements
        if model_name == 'workout_difficulty':
            min_data_points = 50
        elif model_name == 'workout_ranking':
            min_data_points = 50
        ...
        # Filter by model name and recent data (last 30 days)
        recent_date = datetime.now() - timedelta(days=30)
        
        relevant_feedback = [
            entry for entry in all_feedback 
            if entry['model_name'] == model_name and 
            datetime.fromisoformat(entry['timestamp']) > recent_date
        ]
        
        relevant_outcomes = [
            entry for entry in all_outcomes 
            if entry['model_name'] == model_name and 
            datetime.fromisoformat(entry['timestamp']) > recent_date
        ]
    ...
    def _validate_model(self, model, model_name: str, X_val: np.ndarray, y_val: np.ndarray) -> Dict[str, float]:
        ...
        if model_name == 'workout_difficulty':
            ...
            try:
                ...
                auc = 0.0  # AUC not applicable for multiclass without probabilities
    ...
    def _compare_models(self, model_name: str, current_metrics: Dict[str, float], 
        ...
        # For classification models, higher accuracy/AUC is better
        # For regression models, lower MSE/RMSE is better
        
        if model_name in ['workout_difficulty', 'nutrition_adherence']:
            # Classification - higher is better
            current_score = current_metrics.get('accuracy', 0) + current_metrics.get('auc', 0)
            new_score = new_metrics.get('accuracy', 0) + new_metrics.get('auc', 0)
        elif model_name == 'workout_ranking':
            # Regression - lower is better (but we'll use negative for comparison)
            current_score = -(current_metrics.get('rmse', float('inf')))
            new_score = -(new_metrics.get('rmse', float('inf')))
        else:
            return False
...
class LearningImprovementScheduler:
    ...
    def run_periodic_retraining(self) -> Dict[str, bool]:
        ...
        for model_name in model_names:
            self.logger.info(f"Checking retraining for {model_name}")
