...
class RecommendationPipeline:
    ...
    def apply_rule_based_safety_filters(self, candidates: List[Dict[str, Any]],
        ...
        for candidate in candidates:
            ...
            if rec_type == RecommendationType.WORKOUT:
                ...
                meal_ingredients = [ing.lower() for ing in candidate.get('ingredients', [])]
                meal_name_lower = candidate.get('name', '').lower()
                ...
                if is_safe:
                    for restriction in dietary_restrictions:
                        if restriction == 'vegan':
                            ...
                            animal_products = ['meat', 'fish', 'chicken', 'beef', 'pork', 'turkey', 'duck',
                                             'seafood', 'shrimp', 'crab', 'lobster', 'eggs', 'dairy',
                                             'milk', 'cheese', 'yogurt', 'butter', 'cream', 'honey',
                                             'gelatin', 'lard', 'collagen']
                            if any(product in meal_ingredients for product in animal_products):
                                is_safe = False
                                break
                        elif restriction == 'vegetarian':
                            # Vegetarian restrictions: no meat/fish/poultry
                            meat_products = ['meat', 'fish', 'chicken', 'beef', 'pork', 'turkey', 'duck',
                                           'seafood', 'shrimp', 'crab', 'lobster']
                            if any(product in meal_ingredients for product in meat_products):
                                is_safe = False
                                break
                        elif restriction == 'gluten_free':
                            # Gluten-free restrictions
                            gluten_sources = ['wheat', 'barley', 'rye', 'oats_unspecified', 'bread', 'pasta',
                                            'flour', 'semolina', 'bulgur', 'durum', 'farina', 'graham',
                                            'spelt', 'triticale']
                            if any(source in meal_ingredients for source in gluten_sources):
                                is_safe = False
                                break
                        elif restriction == 'dairy_free':
                            # Dairy-free restrictions
                            dairy_products = ['milk', 'cheese', 'yogurt', 'butter', 'cream', 'ice_cream',
                                            'whey', 'casein', 'lactose']
                            if any(product in meal_ingredients for product in dairy_products):
                                is_safe = False
                                break
    ...
    def apply_equipment_compatibility_filters(self, candidates: List[Dict[str, Any]], 
        ...
        """
        if rec_type == RecommendationType.MEAL:
            # Meals don't require equipment, so return all candidates
            return candidates
    ...
    def apply_injury_and_experience_constraints(self, candidates: List[Dict[str, Any]], 
        ...
        """
        if rec_type == RecommendationType.MEAL:
            # For meals, we don't apply experience or injury constraints in the same way
            # We already handled dietary restrictions in step 1
            return candidates
    ...
    def re_validate_final_output(self, candidates: List[Dict[str, Any]], 
        ...
        for candidate in candidates:
            is_valid = self._validate_single_item(candidate, user_profile, rec_type)
    ...
    def _validate_single_item(self, item: Dict[str, Any], user_profile: Dict[str, Any], 
                            rec_type: RecommendationType) -> bool:
        """
        ...
        step1 = self.apply_rule_based_safety_filters(temp_list, user_profile, rec_type)
        step2 = self.apply_equipment_compatibility_filters(step1, user_profile, rec_type)
        step3 = self.apply_injury_and_experience_constraints(step2, user_profile, rec_type)
    ...
    def generate_recommendation(self, candidates: List[Dict[str, Any]], 
        ...
        # Step 1: Apply rule-based safety filters
        step1_result = self.apply_rule_based_safety_filters(candidates, user_profile, rec_type)
        print(f"  Step 1 - Safety filters: {len(candidates)} → {len(step1_result)} items")
        
        # Step 2: Apply equipment compatibility filters
        step2_result = self.apply_equipment_compatibility_filters(step1_result, user_profile, rec_type)
        print(f"  Step 2 - Equipment filters: {len(step1_result)} → {len(step2_result)} items")
        
        # Step 3: Apply injury and experience constraints
        step3_result = self.apply_injury_and_experience_constraints(step2_result, user_profile, rec_type)
        ...
        # Step 5: Re-validate final output against safety rules
        final_result = self.re_validate_final_output(step4_result, user_profile, rec_type)
        ...
        for item in final_result:
            if self._validate_single_item(item, user_profile, rec_type):
...
class MockMealAdherenceModel:
    ...
    def rank_meals(self, meals: List[Dict[str, Any]], user_profile: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for meal in meals:
            ...
            for restriction in dietary_restrictions:
                if restriction == 'vegan' and any(ing in ['meat', 'fish', 'eggs', 'dairy', 'honey'] for ing in meal_ingredients):
                    has_restriction_violation = True
                    break
                elif restriction == 'vegetarian' and any(ing in ['meat', 'fish'] for ing in meal_ingredients):
                    has_restriction_violation = True
                    break
                elif restriction == 'gluten_free' and any(ing in ['wheat', 'barley', 'rye', 'oats_unspecified'] for ing in meal_ingredients):
                    has_restriction_violation = True
                    break
                elif restriction == 'dairy_free' and any(ing in ['milk', 'cheese', 'yogurt', 'butter', 'cream'] for ing in meal_ingredients):
                    has_restriction_violation = True
                    break
...
def main():
    ...
    for i, meal in enumerate(meal_rec, 1):
        print(f"  {i}. {meal['name']} - {meal['calories']} cal, {meal['protein_g']}g protein")
    
    print("\n" + "="*50)
    print("PIPELINE VALIDATION:")
    print("✅ Rule-based safety filters applied first")
    print("✅ Equipment compatibility checked (for workouts)")
    print("✅ Injury and experience constraints enforced")
    print("✅ Dietary restrictions and allergies checked (for meals)")
