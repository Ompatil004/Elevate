...
class MLService:
    ...
    def load_data(self):
        ...
        if os.path.exists(self.ex_processed):
            ...
            self.df_ex = self.df_ex.rename(columns={'equipment': 'Equipment', 'name': 'Name'})
        ...
        if not self.df_ex.empty:
            self.df_ex = self.df_ex.fillna({'Avoid_If': 'None', 'Check_Type': 'general_logic', 'Progression_Next': ''})
        if not self.df_nut.empty:
            self.df_nut = self.df_nut.fillna({'Allergens': 'None', 'Swap_Group': 'Generic'})
    ...
    def recommend_meals(self, goal, preference, allergies, target_calories):
        ...
        for meal_type, ratio in targets.items():
            ...
            candidates = pool[pool['Type'].str.contains(meal_type, case=False, na=False)].copy()
