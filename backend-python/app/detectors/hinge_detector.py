...
class HingeDetector(BaseDetector):
    """
    Detector for hinge movements (deadlifts, good mornings).
    Tracks hip angle.
    """
    def count_reps(self, landmarks, state_data: Dict[str, Any]) -> Tuple[int, str]:
        ...
        down_threshold = self.config.get('down_angle', 120.0)
        up_threshold = self.config.get('up_angle', 160.0)
        rom_min = self.config.get('rom_min', 35.0)
        ...
        if stage == 'up':
            ...
            min_angle = min(min_angle, hip_angle)
    ...
    def check_form(self, landmarks) -> List[str]:
        ...
        if l_shoulder and l_hip and l_knee and l_ankle:
            ...
            if hip_angle < 100 and knee_angle < 110:
                self.feedback.append("Keep legs straighter - this is a hip hinge, not a squat")
