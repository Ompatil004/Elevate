...
import { saveProfile, getProfile, generateAIPlan } from '../api';
import { updateProfileWithRegeneration } from '../services/profileApi';
import { removeFromStorage, StorageKeys } from '../utils/storage';
import '../App.css';
...
const styles = {
  container: {
    ...
    padding: '20px 20px', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Inter', sans-serif"
  },
  ...
  wrapper: { maxWidth: '1100px', width: '100%', position: 'relative', zIndex: 10, marginTop: '10px' },
  ...
  headerRow: { display: 'flex', gap: '20px', alignItems: 'center', marginBottom: '20px', borderBottom: '2px dashed #e2e8f0', paddingBottom: '20px' },
...
function ProfileSetup() {
  ...
  const clearWorkoutPlanCache = () => {
    try {
      // Remove cached enriched data
      removeFromStorage(StorageKeys.CACHED_ENRICHED_DATA);
      removeFromStorage(StorageKeys.CACHED_ENRICHED_DATA_TIMESTAMP);

      // Remove any other workout-related cached data
  ...
  const handleSubmit = async (e) => {
    ...
    if (!firstName.trim()) {
        showError("Please enter your first name!", 3000);
    ...
    try {
        ...
        // 2. Send to Backend - use the function that calls backend-python server where regeneration logic exists
        // Include user ID to identify which user to update
        const token = localStorage.getItem('token');
        // Decode JWT token to get user ID (simplified - in real app you'd properly decode JWT)
        let userId = 'unknown';
        if (token) {
          try {
            const payload = JSON.parse(atob(token.split('.')[1]));
            userId = payload.user.id || payload.userId || 'unknown';
          } catch (e) {
        ...
        const response = await updateProfileWithRegeneration(profileUpdateData);

        // 3. Clear workout plan cache to force regeneration after profile update
        // This ensures stale cached plans are discarded and new plans are generated
        clearWorkoutPlanCache();
        console.log("🔄 Workout plan cache cleared - will regenerate on next visit");

        // 4. Generate AI Plan (only for new setup, not for editing)
        if (!isEditing) {
          try {
            await generateAIPlan(finalData);
            showSuccess("AI Plan Generated Successfully! Welcome to Elevate.", 3000);
          } catch (planError) {
            console.error("Error generating AI plan:", planError);
            // Still allow navigation even if plan generation fails
            showSuccess("Profile saved, but there was an issue generating your AI plan. You can refresh the dashboard later.", 5000);
          }
        } else {
          showSuccess("Profile Updated Successfully!", 3000);
        }
        ...
        showError("Failed to save: " + errorMsg, 5000);
    } finally {
        setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {isEditing ? (
        ...
        <button onClick={() => { localStorage.clear(); navigate('/'); }} style={styles.cancelBtn}>
          LOGOUT
        </button>
      )}
